...
<body class="bg-gray-100">
    <div class="container mx-auto p-4">
        ...
        @endif
        <form method="POST" action="{{ route('reviews.update', $review) }}" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div class="mb-4">
                ...
                <select id="id_film" name="id_film" class="w-full p-2 border rounded" required>
                    @foreach ($films as $film)
                        <option value="{{ $film->id_film }}" {{ $film->id_film == $review->id_film ? 'selected' : '' }}>{{ $film->judul }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                <label for="nilai" class="block text-sm font-medium">Rating (1-10)</label>
                <input id="nilai" type="number" name="nilai" value="{{ $review->nilai }}" min="1" max="10" class="w-full p-2 border rounded" required>
            </div>
            <div class="mb-4">
                <label for="komentar" class="block text-sm font-medium">Comment</label>
                <textarea id="komentar" name="komentar" class="w-full p-2 border rounded">{{ $review->komentar }}</textarea>
            </div>
